...
NCI_STORY_BLOCK = PROJECT / "03_SCRIPT_BLOCKS" / "10_nci_story_layer_v0_1_2_strong_ob_story.pine"
ALERT_BLOCK = PROJECT / "03_SCRIPT_BLOCKS" / "11_three_ok_alerts_v0_1_4.pine"
OUT = PROJECT / "03_CANDIDATES" / "master-smc-sats-nci-story-v0.1.4.pine"
...
def make_easy_wording(block: str) -> str:
    block = block.replace("v0.1.2 STRONG OB STORY", "v0.1.4 EASY STORY + ALERTS")
    block = block.replace("v0.1.2", "v0.1.4")
    replacements = {
    ...
    def repl_line(line: str) -> str:
        ...
        if in_str:
            out.append('"' + "".join(buf))
        return "".join(out)
    return "\n".join(repl_line(line) for line in block.splitlines())
...
def main():
    ...
    story = make_easy_wording(NCI_STORY_BLOCK.read_text(encoding="utf-8"))
    alerts = ALERT_BLOCK.read_text(encoding="utf-8")

    for name, block in [("story", story), ("alerts", alerts)]:
        if count_active(block, "//@version") != 0 or count_active(block, "indicator(") != 0:
            raise RuntimeError(f"{name} block must not contain active version or indicator declaration")
    ...
    for required in [
        ...
        "TQI",
        "NCI STORY LAYER",
        "THREE OK DASHBOARD ALERTS",
        "nciBuyThreeOkAlert",
        "nciSellThreeOkAlert",
        "alertcondition(nciBuyThreeOkAlert",
        "alertcondition(nciSellThreeOkAlert",
        "v0.1.4",
    ]:
